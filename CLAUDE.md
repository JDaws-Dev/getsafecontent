...
<!--
  Keep this section short — 5-10 lines max. Move historical session detail
  to docs/BUILD-HISTORY.md. This block loads into every Claude turn, so
  bloat here costs you context tokens forever. Pruning discipline added
  2026-05-28 after the footer hit 73 KB on a single line.
-->
...
- Cross-app kid login nav on all 5 apps (one tap to hop, `?fc=` auto-fills code)
- Admin `/admin/*` accepts Marketing Central password sign-in (Google OAuth fallback still works)
