...
function ResetPasswordPage() {
  ...
  useEffect(() => {
    if (success && isAuthenticated && user) {
      ...
      if (user.entitledApps?.includes('safetube')) {
        navigate('/admin');
      }
    }
          ...
          <div className="bg-white rounded-xl shadow-lg p-8">
            {success ? (
              <div className="text-center">
                ...
                <Link
                  to="/admin"
                  className="inline-block bg-accent-500 hover:bg-accent-600 text-white px-6 py-2 rounded-lg font-semibold transition"
                >
                  Go to Dashboard
                </Link>
              ...
              <>
                    ...
                    <div className="flex justify-center gap-2" onPaste={handlePaste}>
                      {code.map((digit, index) => (
                        <input
                          ...
                          className="w-12 h-14 text-center text-2xl font-bold border border-gray-300 rounded-lg focus:ring-2 focus:ring-accent-500 focus:border-transparent"
                          autoFocus={index === 0}
                        />
                      ))}
                    </div>
                ...
                <div className="mt-6 text-center">
                  <p className="text-sm text-gray-600">
                    ...
                    <Link to="/forgot-password" className="text-accent-500 hover:text-accent-600 font-medium">
                      Request new code
                    </Link>
                  </p>
                </div>
