...
/**
 * SafeTube server-side identity — the ONLY trustworthy answer to "who is
 ...
 *
 * Mirrors SafeReads / SafeStudy / SafeTunes (see apps/safereads/convex/identity.ts).
 * Parents authenticate with a Marketing Central JWT (the `safetube_jwt` the
 * frontend already holds). The KID path (KidPlayer) authenticates by family code
...
// Thin wrapper that supplies SafeTube's mirrored secret.
...
export async function requireOwner(
  ...
  const me = await resolveTubeIdentity(ctx, userToken);
...
export async function requireOwnerSoft(
  ...
  if (!userToken) {
    console.warn(`[safetube auth] UNVERIFIED ${label} for user ${ownerId} (no token; pre-migration)`);
    return;
  }
 ...
 * local `users` row. The frontend calls this on login so SafeTube always tracks
