...
export function AuthorCard({ author }: AuthorCardProps) {
  ...
  return (
    <Link
      ...
      className="group flex gap-4 rounded-2xl border border-brand-cream-2 bg-brand-cream p-4 transition-colors hover:border-accent-400 hover:shadow-sm"
    >
      {/* Author avatar / icon */}
      ...
      <div className="min-w-0 flex-1">
        ...
        <h3 className="mt-1 font-display text-base font-bold text-brand-navy group-hover:text-accent-700">
          {author.name}
        </h3>
        ...
        {author.topBooks.length > 0 && (
          <div className="mt-2 flex gap-1.5">
            {author.topBooks.slice(0, 4).map((book, i) => (
              <div
                key={i}
                className="relative h-12 w-8 flex-shrink-0 overflow-hidden rounded bg-brand-cream-2"
              >
                {book.coverUrl ? (
                  <Image
                    src={book.coverUrl}
                  ...
                  <div className="flex h-full items-center justify-center">
                    <BookOpen className="h-3 w-3 text-accent-300" />
                  </div>
                )}
              </div>
            ))}
            {author.bookCount > 4 && (
              <div className="flex h-12 w-8 flex-shrink-0 items-center justify-center rounded bg-brand-cream-2 text-xs text-ink-400">
                +{author.bookCount - 4}
              </div>
            )}
          </div>
