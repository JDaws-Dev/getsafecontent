 ...
 *
 * Usage:
 *   POST https://exuberant-puffin-838.convex.site/updatePassword?key=xxx
 *   Body: {
 *     email: string,
...
export default httpAction(async (ctx, request): Promise<Response> => {
  ...
  try {
    ...
    const result = await ctx.runMutation(internal.updatePasswordInternal.updatePasswordInternal, {
      email: body.email,
