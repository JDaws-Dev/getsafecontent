...
export const processTrialExpirations = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    for (const user of users) {
      ...
      // Compute trial end: trialExpiresAt field, or createdAt + 7 days
      const trialEnd = user.trialExpiresAt ?? ((user.createdAt ?? user._creationTime) + SEVEN_DAYS_MS);
    ...
    if (skippedAncient > 0) {
      console.log(`[SafeReads] Expired ${skippedAncient} ancient trials silently (30+ days old)`);
    }
