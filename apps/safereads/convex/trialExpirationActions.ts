  ...
  handler: async (ctx) => {
    ...
    for (const user of expiringSoon) {
      try {
        ...
        console.error(`[SafeReads] Failed to send warning email to ${user.email}:`, e);
      }
    }
    ...
    for (const user of expired) {
      try {
        ...
        console.error(`[SafeReads] Failed to send expired email to ${user.email}:`, e);
      }
    }
    ...
    if (expired.length > 0 || expiringSoon.length > 0) {
      try {
        ...
        console.error("[SafeReads] Failed to send admin trial summary:", e);
      }
    }

    console.log(
      `[SafeReads] Trial check complete: ${expired.length} expired, ${expiringSoon.length} warnings sent`
    );
  },
});
