...
/**
 * SafeStudy server-side identity — the ONLY trustworthy answer to "who is
 ...
 * `userId`/`kidProfileId` (which any caller can forge — the IDOR this fixes).
 *
 * Mirrors SafeReads' proven pattern (see apps/safereads/convex/identity.ts).
 * Parents authenticate with a Marketing Central JWT (the `safestudy_jwt` the
 * frontend already holds). The KID path (KidSearch) authenticates by family code
 * with no parent token — those calls intentionally stay on `requireOwnerSoft`
 * (warn + allow) until kids get their own scoped session token in a later phase.
 */
...
// Thin wrapper that supplies SafeStudy's mirrored secret.
...
/**
 * Verify the caller's Marketing JWT and resolve it to the SafeStudy `users`
 ...
 * When no token is present (old frontend build still in flight, or the kid-side
 * family-code path) it logs the unverified access and allows it, so nothing
 * breaks while the frontend is updated to pass `userToken`. When a token IS
 * supplied it is fully enforced — so an attacker can't downgrade by omitting the
 * token AND we get telemetry on how many real calls are still tokenless before
 * we flip to hard `requireOwner`.
 */
export async function requireOwnerSoft(
  ...
  if (!userToken) {
    console.warn(`[safestudy auth] UNVERIFIED ${label} for user ${ownerId} (no token; pre-migration)`);
    return;
  }
 ...
 * local `users` row. The frontend calls this on login so SafeStudy always
 * tracks Central's one canonical code (docs/UNIFIED-IDENTITY.md) — replacing
 * the local `generateUniqueFamilyCode()` fallback that caused drift. Idempotent;
 * no-op when unchanged. Requires MARKETING_JWT_SECRET to be set, else
 * verification fails closed and this throws (frontend treats as "log in again").
 */
