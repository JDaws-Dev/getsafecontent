...
import { SafeFamilyParentSwitcher } from '../SafeFamilySwitcher';
  ...
  return (
    <button
      ...
      className={`w-full bg-white rounded-xl p-4 border transition hover:shadow-md text-left ${
        isBlocked ? 'border-red-200 bg-red-50/50' : 'border-gray-100 hover:border-accent-200'
      }`}
    >
        ...
        <div className="flex-1 min-w-0">
          ...
          {lastSong ? (
            <p className="text-sm text-gray-500 truncate flex items-center gap-1">
              <svg className="w-3.5 h-3.5 text-accent-500 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                <path d="M18 3a1 1 0 00-1.196-.98l-8 1.6A1 1 0 008 4.6v7.116A3.5 3.5 0 1010 14.5V8.82l6-1.2v3.096A3.5 3.5 0 1018 14V3z" />
              </svg>
              <span className="truncate">"{lastSong.itemName}" · {getRelativeTime(lastSong.playedAt)}</span>
            </p>
          ) : (
    ...
    <>
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[80vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex justify-between items-center p-6 border-b border-gray-200">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-xl font-display font-bold text-brand-navy">{kid.name}'s Listening History</h3>
            ...
            {/* Stats Summary */}
            <div className="px-6 py-4 bg-accent-50 border-b border-gray-200">
              <div className="grid grid-cols-3 gap-4 text-center">
                <div>
                  <div className="text-2xl font-bold text-accent-600">{listeningStats?.totalPlays || 0}</div>
                  <div className="text-xs text-gray-600">Total Plays</div>
                </div>
                <div>
                  <div className="text-2xl font-bold text-accent-600">{listeningStats?.uniqueSongs || 0}</div>
                  <div className="text-xs text-gray-600">Unique Songs</div>
                </div>
                <div>
                  <div className="text-2xl font-bold text-accent-600">{formatListenTime(listeningStats)}</div>
                    ...
                    {allRecentSongs.map((item, index) => (
                      <div key={item._id} className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition">
                        ...
                        {item.artworkUrl ? (
                          ...
                          <div className="w-12 h-12 rounded bg-accent-100 flex items-center justify-center flex-shrink-0">
                        ...
                        <div className="text-right flex-shrink-0">
                          ...
                          {item.playCount > 1 && (
                            <div className="text-xs text-accent-600 font-medium">{item.playCount}x played</div>
                          )}
                        </div>
                      </div>
                    ))}
                  ...
                  <div className="space-y-2">
                    {listeningStats.topSongs.map((item, index) => (
                      <div key={item._id} className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition">
                        ...
                        <div className="text-right flex-shrink-0">
                          <div className="text-lg font-bold text-accent-600">{item.playCount}</div>
                          <div className="text-xs text-gray-500">plays</div>
                        </div>
                      </div>
                    ))}
                  ...
                  <div className="space-y-3">
                    {listeningStats.topArtists.map((artist, index) => (
                      <div key={artist.name} className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition">
                        ...
                        <div className="text-right flex-shrink-0">
                          <div className="text-lg font-bold text-accent-600">{artist.count}</div>
                          <div className="text-xs text-gray-500">plays</div>
                        </div>
                      </div>
                    ))}
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div className="bg-accent-50 rounded-lg p-3">
          <div className="text-2xl font-bold text-accent-600">{kidAlbums.length}</div>
          <div className="text-xs text-gray-600 mt-1">Albums</div>
        </div>
        <div className="bg-accent-50 rounded-lg p-3">
          <div className="text-2xl font-bold text-accent-600">{kidTracks}</div>
      ...
      {listeningStats && listeningStats.totalPlays > 0 && (
        <div className="bg-gradient-to-r bg-accent-50 rounded-lg p-3 mb-4">
          ...
          <div className="space-y-2">
            {recentSongs.map((item) => (
              <div key={item._id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-50 transition">
                ...
                {item.playCount > 1 && (
                  <span className="text-xs text-accent-600 font-medium bg-accent-50 px-2 py-0.5 rounded-full">
                    {item.playCount}x
                  </span>
                )}
              </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center justify-between py-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-xl font-display font-bold text-brand-navy">SafeTunes</h1>
                <p className="text-xs text-gray-600">Parent Dashboard</p>
              </div>
            </div>
            <div className="flex items-center space-x-2">
              {/* Safe Family — always-visible one-tap app switcher (desktop; links to sibling parent dashboards) */}
              <div className="hidden lg:flex">
                <SafeFamilyParentSwitcher current="safetunes" familyCode={fullUser?.familyCode} />
              </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-24 md:pb-8">
        ...
        {activeTab === 'kids' && (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-2xl font-display font-bold text-brand-navy">Kids' Listening Habits</h2>
                ...
                {kidProfiles.map((kid) => {
                  ...
                  return (
                    <div key={kid._id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                      {/* Kid Header */}
                      <div className="bg-accent-50 p-6 border-b border-gray-100">
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex-1">
                            <h3 className="text-2xl font-display font-bold text-brand-navy">{kid.name}</h3>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-6 bg-gray-50">
                        <div className="text-center">
                          <div className="text-3xl font-bold text-accent-600">{kidAlbums.length}</div>
                          <div className="text-sm text-gray-600 mt-1">Albums</div>
                        </div>
                        <div className="text-center">
                          <div className="text-3xl font-bold text-accent-600">{kidTracks}</div>
                          <div className="text-sm text-gray-600 mt-1">Tracks</div>
                        </div>
                        <div className="text-center">
                          <div className="text-3xl font-bold text-accent-600">{kid.favoriteGenres?.length || 0}</div>
                                ...
                                <div className="aspect-square rounded-lg overflow-hidden bg-gray-100 shadow-md group-hover:shadow-lg transition">
                                  {album.artworkUrl ? (
                                    ...
                                    <div className="w-full h-full flex items-center justify-center bg-accent-100">
                                      <svg className="w-12 h-12 text-accent-300" fill="currentColor" viewBox="0 0 20 20">
                                        <path d="M18 3a1 1 0 00-1.196-.98l-10 2A1 1 0 006 5v9.114A4.369 4.369 0 005 14c-1.657 0-3 .895-3 2s1.343 2 3 2 3-.895 3-2V7.82l8-1.6v5.894A4.37 4.37 0 0015 12c-1.657 0-3 .895-3 2s1.343 2 3 2 3-.895 3-2V3z" />
                                      </svg>
                                    </div>
                                  )}
                        ...
                        <div className="px-6 pb-6">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {kid.favoriteGenres.map((genre) => (
                              <span key={genre} className="px-3 py-1 bg-accent-100 text-accent-700 rounded-full text-sm font-medium">
                                {genre}
                              </span>
                            ))}
                          </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full">
            ...
            <div className="p-6 border-b border-gray-200">
              <h3 className="text-xl font-display font-bold text-brand-navy">Remove Album</h3>
