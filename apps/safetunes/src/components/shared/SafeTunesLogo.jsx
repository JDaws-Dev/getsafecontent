/**
 * SafeTunes Shield Logo Component
 * Used for hidden artwork placeholders throughout the app
 */

// SafeTunes Shield Logo SVG
export function SafeTunesLogo({ className = "w-6 h-6" }) {
  return (
...
// Hidden Artwork Placeholder with SafeTunes branding
export function HiddenArtworkPlaceholder({
  size = 'md', // 'sm' (32px), 'md' (48px), 'lg' (80px), 'xl' (120px)
  className = ''
}) {
  const sizeClasses = {
    sm: 'w-8 h-8',
  ...
  return (
    <div className={`${sizeClasses[size]} bg-gradient-to-br from-gray-400 to-gray-500 rounded-lg flex items-center justify-center shadow-sm ${className}`}>
      <SafeTunesLogo className={`${logoSizes[size]} text-white/70`} />
    </div>
  );
}

export default SafeTunesLogo;
