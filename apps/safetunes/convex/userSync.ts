...
export const ensureSafeTunesUser = mutation({
  ...
  handler: async (ctx, args) => {
    // Check if SafeTunes user already exists
    const existing = await ctx.db
      .query("users")
      .withIndex("email", (q) => q.eq("email", args.email))
      .first();
    ...
    console.log(`[ensureSafeTunesUser] Created missing user: ${args.email} -> ${userId}`);
...
export const updateSafeTunesUserSubscription = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!user) {
      throw new Error("SafeTunes user not found for email: " + args.email);
    }
