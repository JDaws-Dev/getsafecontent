  ...
  handler: async (ctx, args) => {
    ...
    try {
      const result = await resend.emails.send({
        from: "SafeTunes <noreply@getsafefamily.com>",
...
export const sendBatchedRequestNotification = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      const result = await resend.emails.send({
        from: "SafeTunes <notifications@getsafefamily.com>",
        replyTo: "jeremiah@getsafefamily.com",
        to: args.userEmail,
        subject: isMultiple
...
/**
 * Send orphan alert email to admin when orphaned records are detected
 * Called by the daily orphan detection cron job
 */
export const sendOrphanAlertEmail = action({
  args: {
    totalOrphans: v.number(),
    summary: v.record(v.string(), v.number()),
    sampleRecords: v.array(v.object({
      table: v.string(),
      recordId: v.string(),
      missingParentId: v.string(),
      recordInfo: v.string(),
  ...
  handler: async (ctx, args) => {
    ...
    const summaryRows = Object.entries(args.summary)
      .map(([table, count]) => `<tr><td style="padding: 8px; border: 1px solid #e5e7eb;">${table}</td><td style="padding: 8px; border: 1px solid #e5e7eb; text-align: center;">${count}</td></tr>`)
      .join("");

    // Build sample records list
    const sampleRecordsList = args.sampleRecords
      .map((record) => {
        const date = record.createdAt ? new Date(record.createdAt).toLocaleDateString() : "Unknown";
        return `<li style="margin-bottom: 8px;">
          <strong>${record.table}</strong>: ${record.recordInfo}<br>
          <span style="color: #6b7280; font-size: 12px;">ID: ${record.recordId} | Missing parent: ${record.missingParentId} | Created: ${date}</span>
        </li>`;
      })
      .join("");
    ...
    const emailContent = `
      ...
      <html>
        ...
        <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <!-- Summary Table -->
          <h2 style="margin: 0 0 16px 0; font-size: 18px; color: #1f2937;">Summary by Table</h2>
          <table style="width: 100%; border-collapse: collapse; margin-bottom: 24px;">
            <thead>
              <tr style="background: #f9fafb;">
                <th style="padding: 8px; border: 1px solid #e5e7eb; text-align: left;">Table</th>
                <th style="padding: 8px; border: 1px solid #e5e7eb; text-align: center;">Orphaned Records</th>
              </tr>
            </thead>
            <tbody>
              ${summaryRows}
          ...
          <!-- Sample Records -->
          <h2 style="margin: 0 0 16px 0; font-size: 18px; color: #1f2937;">Sample Records (first 10)</h2>
          ...
          <!-- Action Buttons -->
          <div style="text-align: center; margin-bottom: 24px;">
            <a href="https://formal-chihuahua-623.convex.site/adminOrphans?key=${encodeURIComponent(process.env.ADMIN_KEY || "")}"
               style="display: inline-block; background: #7C3AED; color: white; padding: 14px 32px; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 16px; margin-right: 8px;">
              View All Orphans
            </a>
          </div>
          ...
          <div style="background: #F3F4F6; border-radius: 8px; padding: 16px; margin-bottom: 24px;">
            <h3 style="margin: 0 0 8px 0; font-size: 14px; color: #374151;">Cleanup Options:</h3>
            <ol style="margin: 0; padding-left: 20px; font-size: 14px; color: #4b5563;">
              <li>Review orphans via the admin endpoint above</li>
              <li>Run <code>npx convex run orphanDetection:deleteOrphanedRecords</code> to clean up</li>
          ...
          <div style="text-align: center; padding-top: 24px; border-top: 1px solid #e5e7eb;">
            <p style="color: #6b7280; font-size: 14px; margin: 0;">
              This is an automated alert from the SafeTunes orphan detection system.
            </p>
            <p style="color: #9ca3af; font-size: 12px; margin: 8px 0 0 0;">
              Check runs daily at 4:00 AM UTC
            </p>
          </div>
    ...
    try {
      ...
      console.log(`Orphan alert email sent:`, result);
      return { success: true, id: result.data?.id };
    } catch (error) {
