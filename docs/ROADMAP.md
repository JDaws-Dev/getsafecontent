...
**This week (June 2-9):**
- [ ] **Push the two SafeSpark catch-up commits** (`645bb18e` parent dashboard, `56354223` kid-side workbench). Both sit on local `main` unpushed. Smoke-test sequence in BUILD-HISTORY current-status entry; verify the OpenAI-error → kid-friendly mapping still maps `quota`/`billing`/`insufficient_quota` correctly before push.
- [ ] **Cap Jace's kids at $3/day each** via the now-shipped Phase 2 `setKidDailyBudget` mutation (Knox + Myles in family `4QZ5WP`). Use the cost numbers from the June 2 BUILD-HISTORY entry as the framing for the text to Jace: "Family ran ~230 builds since launch, about $7-15 on my OpenAI bill, capping per-kid going forward so a runaway loop doesn't surprise either of us."
- [ ] **Find Evangeline's signup state** — she's NOT in family `4QZ5WP` (only Knox + Myles). Either she's sharing a profile, started a separate family that's orphaned, or her family-code-entry never completed. Pull `users` rows for any email Jace gives, then either delete-and-restart or attach her to `4QZ5WP`.
- [ ] **SafeSpark viability decision** (one of three). Need one more week of non-beta usage data first. Three paths from June 2 analysis:
  - **(a) SafeSpark standalone tier** outside the unified bundle — $14.99/mo with hard ~150-prompt cap, overage throttles politely. Keeps bundle margin clean for the other 4 apps where AI cost is near-zero.
  - **(b) Crush cost per prompt 3-5×** — gpt-4o-mini default for iteration turns (gpt-4o/gpt-5.5 only on new-project), system prompt strip back to <2KB (memory note `feedback_system_prompt_bloat_kills_creativity.md` says this is overdue anyway), context cap to last 6KB of HTML diff instead of full file. Target $0.030/build → $0.008/build.
  - **(c) Iteration discipline UX** — after N=8 turns on one project, "Spark is getting tired of this one — start fresh?" with one-tap fork. Resets context, drops cost ~40%, kids don't notice.
- [ ] **Git catch-up for the other 4 apps + Marketing.** Same situation as SafeSpark today: features deployed via direct deploys, working trees drifted from HEAD. Threads queued by app: **SafeTunes** (schema additions + emails + orphanDetection refactor + ChildDashboard + CookieConsent), **SafeTube** (channelReview structured JSON — already in prod May 5 per the SafeTube section above), **SafeReads** (users.ts provisioning + BookReader + FreeBookSearch + Navbar), **SafeSeek** (ProfileSelection + ResearchCard + http.ts + .env.prod deletion), **Marketing** (blog page rewrite ~300 lines + new `/api/auth/signup` + signupInternal + FAQ/ProblemSolution/Testimonials tweaks). Plus the **cross-app a11y commit** — same `prefers-reduced-motion` CSS block landed in all 5 apps' `index.css`/`globals.css`; one tiny commit per app, ~15 min total.
