...
export const migrateUserEmail = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    // The password credential itself. `providerAccountId` IS the login
    // identifier for the password provider, and `secret` (the hashed password)
    // lives on the same row — so renaming here moves the login across without
    // touching or resetting her password.
    for (const acct of await ctx.db
      .query("authAccounts")
      .withIndex("userIdAndProvider", (q) =>
        q.eq("userId", target._id).eq("provider", "password")
      )
      .collect()) {
      if (acct.providerAccountId.trim().toLowerCase() === oldEmail) {
        if (!args.dryRun)
          await ctx.db.patch(acct._id, { providerAccountId: newEmail });
        changes.push(`authAccounts/${acct._id}`);
      }
    }

    // Stale reset tokens point at the old address — drop them rather than
    // leave links that would mail the wrong inbox.
    for (const t of await ctx.db.query("passwordResetTokens").collect()) {
      if ((t.email ?? "").trim().toLowerCase() === oldEmail) {
        if (!args.dryRun) await ctx.db.delete(t._id);
        changes.push(`passwordResetTokens/${t._id} (deleted)`);
      }
    }
